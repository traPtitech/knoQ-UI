              ...
              <v-stepper v-model="e1">
                ...
                <v-stepper-items>
                  ...
                  <v-stepper-content step="2">
                    <v-container fluid>
                        ...
                        <v-layout row wrap>
                          <v-flex
                            v-for="member in group.members.slice(
                              (pageSelected - 1) * displaySelectedNum,
                              pageSelected * displaySelectedNum
                            )"
                            :key="member.traq_id"
                            xs12
                            sm6
                      ...
                      <v-pagination
                        ...
                        :length="
                          Math.ceil(group.members.length / displaySelectedNum)
                        "
                        total-visible="5"
                      />
                      <v-text-field v-model="ID" label="traQID" />
                      <v-layout row wrap>
                        <v-flex
                          v-for="member in targetMembers.slice(
                            (page - 1) * displayMemberNum,
                            page * displayMemberNum
                          )"
                          :key="member.traq_id"
                          xs12
                          sm6
                      ...
                      <v-pagination
                        v-model="page"
                        :length="
                          Math.ceil(targetMembers.length / displayMemberNum)
                        "
                        total-visible="5"
                      />
                    </v-container>
                  ...
                  <v-stepper-content step="3">
                    ...
                    <v-btn color="info" @click="patchGroup(group)">保存</v-btn>
                  </v-stepper-content>
                </v-stepper-items>
              </v-stepper>
            </v-form>
...
export default {
  ...
  methods: {
    ...
    async patchGroup(group) {
      try {
        const response = await GroupsRepository.patch(
